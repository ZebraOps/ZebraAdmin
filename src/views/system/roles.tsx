...
interface SysRoleItem { id?: number; name?: string; remark?: string; [key: string]: unknown; }

export default function SystemRoles() {
  const { t } = useTranslation();
  const actionRef = useRef<ActionType>(null);
  const [editRecord, setEditRecord] = useState<SysRoleItem | null>(null);
  ...
  return (
    <>
      <ProTable<SysRoleItem>
        ...
        request={async () => {
          try { const res = await api.fetchRoles({}); return { data: (res as any)?.records ?? [], success: true, total: (res as any)?.total ?? 0 }; }
          catch { return { data: [], success: false, total: 0 }; }
        }}
        headerTitle={t('route.system_roles', { defaultValue: '角色配置' })}
        toolBarRender={() => [
