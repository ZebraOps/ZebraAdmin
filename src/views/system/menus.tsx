...
interface SysMenuItem { id?: number; name?: string; path?: string; [key: string]: unknown; }

export default function SystemMenus() {
  const { t } = useTranslation();
  const actionRef = useRef<ActionType>(null);
  const [editRecord, setEditRecord] = useState<SysMenuItem | null>(null);
  ...
  const columns: ProColumns<SysMenuItem>[] = [
    ...
    { title: t('common.path', { defaultValue: '路径' }), dataIndex: 'path' },
    {
      ...
      render: (_, row) => [
        ...
        <CountdownButton key="del" icon={<DeleteOutlined />} text={t('common.delete', { defaultValue: '删除' })}
          onConfirm={async () => { await api.deleteMenu(row.id!); message.success(t('common.deleteSuccess', { defaultValue: '删除成功' })); actionRef.current?.reload(); }}
        />
      ]
    }
  ];
  ...
  return (
    <>
      <ProTable<SysMenuItem>
        ...
        request={async () => {
          try { const res = await api.fetchMenuTree(); return { data: (res as any)?.data?.list ?? (res as any)?.data ?? [], success: true, total: 0 }; }
          catch { return { data: [], success: false, total: 0 }; }
        }}
        headerTitle={t('route.system_menus', { defaultValue: '菜单配置' })}
        toolBarRender={() => [
      ...
      <ModalForm<SysMenuItem>
        ...
        onFinish={async (values) => {
          try {
            if (editRecord?.id) await api.updateMenu(editRecord.id as number, values as any); else await api.createMenu(values as any);
      ...
      >
        ...
        <ProFormText name="path" label={t('common.path', { defaultValue: '路径' })} />
      </ModalForm>
    </>
  );
}
