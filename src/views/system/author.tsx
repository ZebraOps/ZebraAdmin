...
interface SysAuthorItem { id?: number; name?: string; remark?: string; [key: string]: unknown; }

export default function SystemAuthor() {
  const { t } = useTranslation();
  const actionRef = useRef<ActionType>(null);
  const [editRecord, setEditRecord] = useState<SysAuthorItem | null>(null);
  const [modalOpen, setModalOpen] = useState(false);



  const columns: ProColumns<SysAuthorItem>[] = [
    ...
    { title: t('common.remark', { defaultValue: '备注' }), dataIndex: 'remark' },
    {
      ...
      render: (_, row) => [
        ...
        <CountdownButton key="del" icon={<DeleteOutlined />} text={t('common.delete', { defaultValue: '删除' })}
          onConfirm={async () => { await api.deleteRole(row.id!); message.success(t('common.deleteSuccess', { defaultValue: '删除成功' })); actionRef.current?.reload(); }}
        />
      ]
    }
  ];
  ...
  return (
    <>
      <ProTable<SysAuthorItem>
        rowKey="id" actionRef={actionRef} columns={columns}
        request={async () => {
          try { const res = await api.fetchRoles({}); return { data: (res as any)?.data?.list ?? (res as any)?.data ?? [], success: true, total: 0 }; }
          catch { return { data: [], success: false, total: 0 }; }
        }}
        headerTitle={t('route.system_author', { defaultValue: '授权配置' })}
        toolBarRender={() => [
