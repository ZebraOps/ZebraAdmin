...
import { Button, Popconfirm, Avatar, Tag, Space, Row, Col, message } from 'antd';
...
export default function OrgUsers() {
  ...
  const columns: ProColumns<User>[] = [
      ...
      render: (_, row) => (
        <Space>
          <Avatar size={32} icon={<UserOutlined />} src={row.avatar || undefined}
            style={{ background: 'linear-gradient(135deg, #56cdf3, #719de3)' }} />
          <div>
    ...
    { title: '邮筱', dataIndex: 'email', render: v => v || '—' },
    { title: '手机', dataIndex: 'tel', render: v => v || '—' },
    { title: '部门', dataIndex: 'department', render: v => v || '—' },
    { title: '角色', dataIndex: 'superuser', render: (_, row) => { const m = SUPERUSER_MAP[String((row as any).superuser)] ?? { label: String((row as any).superuser), color: 'default' }; return <Tag color={m.color}>{m.label}</Tag>; } },
    ...
    { title: '操作', key: 'actions', valueType: 'option', fixed: 'right', width: 140,
      render: (_, row) => [
        ...
        <Popconfirm key="del" title="确认删除该用户？" onConfirm={() => api.deleteUser(row.user_id).then(() => { message.success('删除成功'); actionRef.current?.reload(); }).catch(() => message.error('删除失败'))}>
  ...
  return (
    <>
      <ProTable<User>
        ...
        headerTitle="用户管理"
        toolBarRender={() => [<Button key="add" type="primary" icon={<PlusOutlined />} onClick={() => { setEditRecord(null); setModalOpen(true); }}>新增用户</Button>]}
      ...
      >
        ...
        <Row gutter={16}>
          <Col span={12}><ProFormText name="email" label="邮筱" /></Col>
