...
interface FunctionItem { id?: number; name?: string; remark?: string; [key: string]: unknown; }
  ...
  const columns: ProColumns<FunctionItem>[] = [
    ...
    { title: t('common.actions', { defaultValue: '操作' }), key: 'actions', valueType: 'option', fixed: 'right', width: 140,
      render: (_, row) => [
        ...
        <Popconfirm key="del" title={t('common.deleteConfirm', { defaultValue: '确认删除？' })} onConfirm={() => (api as any).deleteFunction(row.id!).then(() => { message.success('删除成功'); actionRef.current?.reload(); })}>
  ...
  return (
    <>
      <ProTable<FunctionItem>
        ...
        request={async () => { try { const res = await (api as any).fetchFunctions({}); return { data: (res as any)?.data?.list ?? (res as any)?.data ?? [], success: true, total: 0 }; } catch { return { data: [], success: false, total: 0 }; } }}
      ...
      <ModalForm<FunctionItem>
        ...
        onFinish={async (values) => {
          try {
            if (editRecord?.id) await (api as any).updateFunction(editRecord.id as number, values); else await (api as any).createFunction(values);
