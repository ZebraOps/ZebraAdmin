  ...
  const columns: ProColumns<Language>[] = [
    ...
    { title: t('common.actions', { defaultValue: '操作' }), key: 'actions', valueType: 'option', fixed: 'right', width: 140,
      render: (_, row) => [
        ...
        hasComp('publish_language_delete') && <Popconfirm key="del" title="确认删除？" onConfirm={async () => { await api.deleteLanguage(row.id!); message.success('删除成功'); actionRef.current?.reload(); }}>
