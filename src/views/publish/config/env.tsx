  ...
  const columns: ProColumns<Environment>[] = [
    ...
    { title: t('common.actions', { defaultValue: '操作' }), key: 'actions', valueType: 'option', fixed: 'right', width: 140,
      render: (_, row) => [
        ...
        hasComp('publish_env_delete') && <Popconfirm key="del" title="确认删除？" onConfirm={async () => { await api.deleteEnvironment(row.id!); message.success('删除成功'); actionRef.current?.reload(); }}>
